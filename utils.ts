...
type TestMyOmit = MyOmit<{
    ...
    description: string
    completed: boolean
}, 'description' | 'title'> // { complected: boolean }

// 12
type Chainable<T = {}> = {
    option<K extends string, V extends any>(key: K, value: V): Chainable<T & Record<K, V>> // 返回Chainable，传入类型为原 类型T + KV键值对
    get (): T
};
...
// 191
type AppendArgument<F extends (...args: any[]) => any, A extends any> = F extends (...args: infer R) => infer X ? 
    (...args: [...R, A]) => X :
    never;
